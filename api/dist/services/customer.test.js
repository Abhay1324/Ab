...
import { getCustomerProfile, createCustomerProfile, updateCustomerProfile, addCustomerAddress, getCustomerAddresses, } from './customer.service.js';
// Arbitrary for valid Indian phone numbers (10 digits starting with 6-9)
const validPhoneArb = fc
    .integer({ min: 6, max: 9 })
    .chain((firstDigit) => fc.integer({ min: 0, max: 999999999 }).map((rest) => {
...
describe('Customer Service', () => {
    ...
    describe('Property 2: Profile Persistence', () => {
        it('for any valid profile data, saving then retrieving should return equivalent data', async () => {
            await fc.assert(fc.asyncProperty(validPhoneArb, validNameArb, fc.option(validEmailArb, { nil: undefined }), async (phone, name, email) => {
                ...
                const savedProfile = await createCustomerProfile(customer.id, profileInput);
        ...
        it('for any valid profile update, changes should persist correctly', async () => {
            await fc.assert(fc.asyncProperty(validPhoneArb, validNameArb, validNameArb, async (phone, initialName, updatedName) => {
                // Clean up
                await prisma.address.deleteMany({ where: { customer: { phone } } });
                await prisma.wallet.deleteMany({ where: { customer: { phone } } });
                await prisma.customer.deleteMany({ where: { phone } });
        ...
        it('for any valid address data, saving then retrieving should return equivalent data', async () => {
            await fc.assert(fc.asyncProperty(validPhoneArb, validNameArb, validAddressArb, async (phone, name, addressInput) => {
                // Clean up
                await prisma.address.deleteMany({ where: { customer: { phone } } });
                await prisma.wallet.deleteMany({ where: { customer: { phone } } });
                await prisma.customer.deleteMany({ where: { phone } });
                ...
                // Add address
                const savedAddress = await addCustomerAddress(customer.id, addressInput);
                // Retrieve addresses
