...
import { sendOTP, verifyOTP, validatePhone, generateOTPCode, } from './otp.service.js';
import { generateTokens, verifyAccessToken, createCustomerSession, } from './token.service.js';
...
describe('OTP Service', () => {
    ...
    beforeEach(async () => {
        // Clean up OTP records before each test
    ...
    describe('Phone Validation', () => {
        it('should validate correct Indian phone numbers', () => {
            fc.assert(fc.property(validPhoneArb, (phone) => {
        ...
        it('should reject invalid phone numbers', () => {
            ...
            fc.assert(fc.property(invalidPhoneArb, (phone) => {
    ...
    describe('Property 1: Authentication Round-Trip', () => {
        it('for any valid phone, sendOTP then verifyOTP with correct code should succeed', async () => {
            await fc.assert(fc.asyncProperty(validPhoneArb, async (phone) => {
        ...
        it('for any valid phone, wrong OTP should fail and track attempts', async () => {
            await fc.assert(fc.asyncProperty(validPhoneArb, otpCodeArb, async (phone, wrongCode) => {
                // Clean up
                await prisma.oTP.deleteMany({ where: { phone } });
                await prisma.wallet.deleteMany({
                    where: { customer: { phone } },
